...
//************************************************************************
{
	...
	else if (in_plane_mode == 2) // Z-slice
	{
		...
		for (k = 0; k < sz; k++)
		{
			a = abs(main_cord_min - p_voxel[io_texture_on_code[k]].x);
			b = abs(main_cord_max - p_voxel[io_texture_on_code[k]].x);
			if (min_ab > abs(a - b))
			{
				if (min_sub > p_voxel[io_texture_on_code[k]].y)
				{
					min_ab = abs(a - b);
					min_sub = p_voxel[io_texture_on_code[k]].y;
					min_idx = k;
				}
			}
		}
